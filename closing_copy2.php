...
$years_query_validasi = "SELECT DISTINCT YEAR(tgl_transaksi) as tahun FROM transaksi WHERE posting = 1 ORDER BY tahun ASC";
...
// Ambil data tahun dari tabel closing untuk bagian Closing
$years_query = "SELECT DISTINCT YEAR(tglclosing) as tahun FROM closing ORDER BY tahun ASC";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action == 'Tampilkan Validasi' && isset($_POST['bulan_validasi']) && isset($_POST['tahun_validasi'])) {
        ...
        $sql_display_validasi = "SELECT detail_transaksi.id_transaksi, detail_transaksi.id_akun, '$tanggal_validasi' as tglclosing, detail_transaksi.nilai
                                 ...
                                 WHERE month(transaksi.tgl_transaksi) = $bulan_validasi 
                                 AND year(transaksi.tgl_transaksi) = $tahun_validasi 
        ...
        // Query untuk menampilkan data dari tabel closing
        $sql_display = "SELECT id, id_akun, tglclosing, saldo as nilai
                        FROM closing 
                        WHERE month(tglclosing) = $bulan 
                        AND year(tglclosing) = $tahun";
